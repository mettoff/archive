 ...
 */
Yii::import('application.modules.project.models.Project');
Yii::import('application.modules.project.events.*');
Yii::import('application.modules.project.listeners.*');
...
class Post extends yupe\models\YModel implements ICommentable
{
    ...
    public function rules()
    {
        ...
        return [
            ['project_id, slug,  title, content, status, publish_time', 'required', 'except' => 'search'],
            [
                'project_id, create_user_id, update_user_id, status, comment_status, access_type, create_time, update_time, category_id',
                'numerical',
                'integerOnly' => true
            ],
            [
                'project_id, create_user_id, update_user_id, create_time, update_time, status, comment_status, access_type',
                'length',
                'max' => 11
            ],
            ['lang', 'length', 'max' => 2],
            ...
            [
                ...
                'message' => Yii::t('ProjectModule.project', 'Forbidden symbols in {attribute}')
            ],
            [
            ...
            ['tags', 'default', 'value' => []],
            [
                'id, project_id, create_user_id, update_user_id, create_time, update_time, slug, publish_time, title, quote, content, link, status, comment_status, access_type, keywords, description, lang',
                'safe',
                'on' => 'search'
            ],
        ];
    ...
    public function attributeLabels()
    {
        return [
            ...
            'update_user_id' => Yii::t('ProjectModule.project', 'Update user'),
            'create_time'    => Yii::t('ProjectModule.project', 'Created at'),
            'update_time'    => Yii::t('ProjectModule.project', 'Updated at'),
            'publish_time'   => Yii::t('ProjectModule.project', 'Date'),
            'slug'           => Yii::t('ProjectModule.project', 'Url'),
            'title'          => Yii::t('ProjectModule.project', 'Title'),
            'quote'          => Yii::t('ProjectModule.project', 'Quote'),
            'content'        => Yii::t('ProjectModule.project', 'Content'),
            'link'           => Yii::t('ProjectModule.project', 'Link'),
            'status'         => Yii::t('ProjectModule.project', 'Status'),
            'comment_status' => Yii::t('ProjectModule.project', 'Comments'),
            'access_type'    => Yii::t('ProjectModule.project', 'Access'),
            'keywords'       => Yii::t('ProjectModule.project', 'Keywords'),
            'description'    => Yii::t('ProjectModule.project', 'description'),
            'tags'           => Yii::t('ProjectModule.project', 'Tags'),
            'image'          => Yii::t('ProjectModule.project', 'Image'),
    ...
    public function search()
    {
        ...
        $criteria->with = ['createUser', 'updateUser', 'project'];
    ...
    public function allPosts()
    {
        ...
        $criteria->with = ['project', 'createUser', 'commentsCount'];
    ...
    public function behaviors()
    {
        ...
        return [
            ...
            'tags'               => [
                ...
                'tagTable'             => Yii::app()->db->tablePrefix . 'project_tag',
                'tagBindingTable'      => Yii::app()->db->tablePrefix . 'project_post_to_tag',
                'tagModel'             => 'Tag',
    ...
    {
        return [
            self::STATUS_DRAFT     => Yii::t('ProjectModule.project', 'Draft'),
            self::STATUS_PUBLISHED => Yii::t('ProjectModule.project', 'Published'),
            self::STATUS_SCHEDULED => Yii::t('ProjectModule.project', 'Scheduled'),
            self::STATUS_MODERATED => Yii::t('ProjectModule.project', 'Moderated'),
    ...
    {
        return [
            self::ACCESS_PRIVATE => Yii::t('ProjectModule.project', 'Private'),
            self::ACCESS_PUBLIC  => Yii::t('ProjectModule.project', 'Public'),
        ];
    }
    ...
    public function getAccessType()
    {
        ...
        return isset($data[$this->access_type]) ? $data[$this->access_type] : Yii::t('ProjectModule.project', '*unknown*');
    }
    ...
    {
        return [
            self::COMMENT_NO  => Yii::t('ProjectModule.project', 'Forbidden'),
            self::COMMENT_YES => Yii::t('ProjectModule.project', 'Allowed'),
        ];
    }
     ...
     * @return mixed
     */
    public function getArchive($projectId = null, $cache = 3600)
    {
        $data = Yii::app()->getCache()->get("Project::Post::archive::{$projectId}");
        ...
        if (false === $data) {
            ...
            Yii::app()->getCache()->set("Project::Post::archive::{$projectId}", $data, (int)$cache);
        }
    ...
    public function getStream($limit = 10, $cacheTime)
    {
        ...
        if (false === $data) {
            ...
            Yii::app()->cache->set('Project::Post::Stream', $data, (int)$cacheTime);
        }
     ...
     * @return mixed
     */
    public function getByTag($tag, array $with = ['project', 'createUser', 'commentsCount'])
    {
    ...
    public function getForProject($projectId)
    {
        ...
        $posts->project_id = (int)$projectId;
    ...
    {
        if (empty($post['project_id']) || empty($post['user_id'])) {
            $this->addError('project_id', Yii::t('ProjectModule.project', "Project is empty!"));

            return false;
        }

        $project = Project::model()->get((int)$post['project_id'], []);

        if (null === $project) {
            $this->addError('project_id', Yii::t('ProjectModule.project', "You can't write in this project!"));

            return false;
        }

        if ($project->isPrivate() && !$project->isOwner($post['user_id'])) {
            $this->addError('project_id', Yii::t('ProjectModule.project', "You can't write in this project!"));

            return false;
        }

        if (!$project->isPrivate() && !$project->userIn($post['user_id'])) {
            $this->addError('project_id', Yii::t('ProjectModule.project', "You can't write in this project!"));
        ...
        $this->status = $post['status'] == self::STATUS_DRAFT ? self::STATUS_DRAFT : $project->post_status;
    ...
    {
        return Yii::app()->createAbsoluteUrl('/project/post/view/', ['slug' => $this->slug]);
    }

...
class PublisherController extends \yupe\components\controllers\FrontController
{
    ...
    {
        $post = new Post();
        $post->project_id = Yii::app()->getRequest()->getParam('project-id');
        ...
        if (Yii::app()->getRequest()->getIsPostRequest() && !empty($_POST['Post'])) {
            ...
            if ($post->createPublicPost($data)) {
                ...
                $message = Yii::t('ProjectModule.project', 'Post sent for moderation!');

                $redirect = ['/project/publisher/my'];

                if ($post->isDraft()) {
                    $message = Yii::t('ProjectModule.project', 'Post saved!');
                }

                if ($post->isPublished()) {

                    $message = Yii::t('ProjectModule.project', 'Post published!');

                    $redirect = ['/project/post/view', 'slug' => $post->slug];
                }
        ...
        $this->render(
            'write',
            ['post' => $post, 'projects' => (new Project())->getListForUser(Yii::app()->getUser()->getId())]
        );
    }

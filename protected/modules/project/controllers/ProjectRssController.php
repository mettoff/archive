...
/**
 * ProjectRssController контроллер для rss на публичной части сайта
 *
 ...
 *
 */
class ProjectRssController extends yupe\components\controllers\RssController
{
    public function loadData()
    {
        ...
        $projectId = (int)Yii::app()->getRequest()->getQuery('project');

        if (!empty($projectId)) {
            $project = Project::model()->cache($yupe->coreCacheTime)->published()->findByPk($projectId);
            if (null === $project) {
                throw new CHttpException(404);
            }
